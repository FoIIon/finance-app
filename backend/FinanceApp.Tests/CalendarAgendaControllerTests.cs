...
public class CalendarAgendaControllerTests : IDisposable
{
    ...
    public async Task Agenda_SemaineParDefaut_FondCalendrierEcheancesEtRecurrentes_AuFuseauDuMenage()
    {
        ...
        Assert.Equal("Danse Clothilde", danse.Title);
    ...
    public async Task Agenda_VueMois_AvecAncre_RepliLesJoursVides()
    {
        ...
        Assert.Equal("Danse Clothilde (déplacée)", Assert.Single(le3.Items).Title);
        ...
        Assert.Equal("Piscine Arsène", Assert.Single(le10.Routine).Title);
        ...
        Assert.Equal("Pas de Danse Clothilde", Assert.Single(le1.Routine).Title);
