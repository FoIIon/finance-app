...
public class TradeRepublicTimelineClassifierTests
{
    ...
    private static readonly string[] Titulaires = ["DUPONT - MARTIN", "Mr SÉBASTIEN DUPONT", "DUPONT S & MARTIN A"];
    ...
    public void Classify_CreditAuNomDuCompteJoint_EstUnVirementInterne()
    {
        ...
        Assert.Equal(TrLineKind.InternalTransfer, Classify("DUPONT - MARTIN"));
    }

    [Fact]
    public void Classify_DepotAuNomDuTitulaire_IgnoreAccentsEtCivilite()
    {
        // Le dépôt de 720 € du 17/08/2026. TR écrit « SEBASTIEN DUPONT », la banque
        // « Mr SÉBASTIEN DUPONT ». Sans normalisation, les deux noms ne se reconnaissent pas.
        Assert.Equal(TrLineKind.InternalTransfer, Classify("SEBASTIEN DUPONT"));
    }

    [Fact]
    public void Classify_LibelleAvecInitialesEtSeparateur_ReconnaitLeCouple()
    {
        // « DUPONT S + MARTIN A » côté banque, « DUPONT S & MARTIN A » côté relevé :
        // les initiales isolées et le séparateur changent, les deux patronymes non.
        Assert.Equal(TrLineKind.InternalTransfer, Classify("DUPONT S + MARTIN A"));
    }
    ...
    public void Classify_CommercantQuiPartageUnSeulPatronyme_ResteUnFlux()
    {
        ...
        Assert.Equal(TrLineKind.Flow, Classify("Boulangerie Dupont"));
    }
    ...
    public void Classify_EventTypeInconnu_RetombeSurLesReglesDeNom()
    {
        ...
        Assert.Equal(TrLineKind.InternalTransfer, Classify("DUPONT - MARTIN", "un_type_que_TR_inventera"));
    ...
    public void Classify_VirementBancaireEntrant_LaisseLeNomDecider()
    {
        ...
        Assert.Equal(TrLineKind.InternalTransfer, Classify("DUPONT - MARTIN", "BANK_TRANSACTION_INCOMING"));
        Assert.Equal(TrLineKind.Flow, Classify("PAPY DUPONT", "BANK_TRANSACTION_INCOMING"));
    }
