...
public class EcheanceReconciliationServiceTests : IDisposable
{
    ...
    public async Task IbanEtMontant_TransactionImportee_RapprocheEtDate()
    {
        ...
        using (var ctx = NewContext())
        {
            ...
            ctx.Echeances.Add(Facture(h, "Repas Clothilde août", 2.60m, new DateOnly(2026, 8, 31)));
    ...
    public async Task LaFenetreDesCandidats_SArreteAuSurlendemain_MemePourUneEcheanceProche()
    {
        ...
        using (var ctx = NewContext())
        {
            ...
            ctx.Transactions.AddRange(
                ...
                tx);
            ctx.Echeances.AddRange(
                Facture(h, "Repas Arsène", 1.40m, Today.AddDays(10)),
                Facture(h, "Repas Clothilde", 2.60m, Today.AddDays(10)));
            await ctx.SaveChangesAsync();
            hier = tx.Id;
        }
        ...
        using var check = NewContext();
        Assert.Null((await check.Echeances.SingleAsync(e => e.Label == "Repas Arsène")).TransactionId);
        Assert.Equal(hier, (await check.Echeances.SingleAsync(e => e.Label == "Repas Clothilde")).TransactionId);
    }
    ...
    public async Task UnePasseQuiRapproche_NeChangeRien_AuBilan()
    {
        ...
        using (var ctx = NewContext())
        {
            ...
            ctx.Echeances.AddRange(
                Facture(h, "Repas Clothilde", 2.60m, new DateOnly(2026, 8, 31)),
                Facture(h, "Repas Arsène", 1.40m, new DateOnly(2026, 8, 31)),
